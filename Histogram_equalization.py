...
data_in = 'C:\\Users\\cheny\\Desktop\\FYP\\LungSegmentation-master\\data_raw\\ct\\LIDC-IDRI-0069.nii'
ct = sitk.ReadImage(data_in, sitk.sitkInt16)
im_orig = sitk.GetArrayFromImage(ct)
#im_orig = util.img_as_float(ct_array)  # grab just the nuclei
...
im_orig_ahe, im_degraded_ahe = \
    [exposure.equalize_adapthist(im,
     ...
     for im in [im_orig, im_degraded]]
im_orig_he = gaussian_filter(im_orig_he, sigma=1)

data_out = 'C:\\Users\\cheny\\Desktop\\FYP\\LungSegmentation-master\\data_modified\\ct\\LIDC-IDRI-0069.nii'
img_out = sitk.GetImageFromArray(im_orig_he)
#img_out.SetDirection(default_direction)
#img_out.SetOrigin(ct.GetOrigin())
#img_out.SetSpacing((2 * rij, 2*rij , 2))
sitk.WriteImage(img_out, data_out)  
